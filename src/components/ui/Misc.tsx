...
const BADGE_TONES: Record<BadgeTone, string> = {
  ...
  ember: 'border-ember/40 text-ember-soft bg-ember/10',
...
export function Badge({
  ...
  return (
    <span
      className={cn(
        'inline-flex items-center gap-1.5 chamfer-xs border px-2.5 py-1 text-xs font-medium',
        BADGE_TONES[tone],
        className,
      )}
    >
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 px-6 py-10 text-center">
      <span className="flex size-14 items-center justify-center chamfer-lg bg-void-700 text-ink-faint">
 ...
 */
const CONTROL_CLASS =
  'w-full min-h-11 chamfer-md border border-void-600 bg-void-900/70 px-3.5 py-2.5 text-sm text-ink placeholder:text-ink-faint transition-colors focus:border-ember focus:outline-none'
...
export function OptionCard({ selected, onSelect, icon, title, description, className }: OptionCardProps) {
  return (
    <button
      ...
      className={cn(
        'flex w-full items-start gap-3 chamfer-md border p-4 text-left transition-all duration-150 active:brightness-95',
        selected
  ...
  return (
    <div
      ...
      className={cn(
        ...
        'max-w-full gap-1 chamfer-md border border-void-600 bg-void-800/70 p-1',
        ...
        return (
          <button
            ...
            className={cn(
              ...
              'tap-target inline-flex items-center justify-center gap-1.5 chamfer-sm font-medium transition-all duration-150',
              ...
              // Rótulo de sistema: Saira condensada, maiúsculas, espaçada. A
              // Saira é mais estreita do que a Barlow, por isso a caixa alta
              // não custa largura em relação ao que aqui estava — o `truncate`
              // continua a ser a rede para a tradução mais comprida.
              'font-display font-bold uppercase',
              fullWidth
                ? 'min-w-0 flex-1 truncate px-1.5 py-2.5 text-[11px] tracking-[0.1em]'
                : 'px-3.5 py-2 text-[13px] tracking-[0.14em]',
              // O separador ativo acende, como o módulo ativo do protótipo:
              // fundo de reiatsu a 12 % e filete de 3 px à esquerda.
              active
                ? 'bg-ember/[0.12] text-ember shadow-[inset_3px_0_0_0_var(--color-ember)]'
...
export function Stat({
  ...
  return (
    <div className="chamfer-md border border-void-600/70 bg-void-900/40 p-3">
      <div className="flex items-center gap-1.5 text-xs font-medium text-ink-muted">
        {icon && <Icon name={icon} size={13} />}
        {label}
      </div>
      <p className={cn('mt-1 font-mono text-2xl font-bold tabular-nums', color)}>{value}</p>
    </div>
  )
}
