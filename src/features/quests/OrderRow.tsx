...
export function OrderRow({ quest, detailed = false }: { quest: Quest; detailed?: boolean }) {
  ...
  return (
    <button
      ...
      onClick={advance}
      className={cn(
        'grid items-center gap-[11px] chamfer-md border px-[13px] text-left transition-colors',
    ...
    >
      <span
        className={cn(
          'flex size-5 items-center justify-center chamfer-xs border-[1.5px] text-xs font-bold',
      ...
      <span className="min-w-0">
        ...
        */}
        ...
        {detailed && (
          <span className="mt-1.5 block h-1 bg-void-700">
            <span
              className={cn('block h-full', done ? 'bg-good': 'bg-ember')}
              style={{ width: `${pct}%` }}
            />
          </span>
        )}
      ...
      {detailed ? (
        <span className="flex shrink-0 gap-[5px]">
          <span className="chamfer-xs bg-ember/[0.14] px-[7px] py-[3px] text-[10.5px] font-bold text-ember-soft">
            +{n(quest.rewardXp)}
          </span>
          <span className="chamfer-xs bg-gold/[0.14] px-[7px] py-[3px] text-[10.5px] font-bold text-gold-soft">
            +{n(quest.rewardCoins)}
          </span>
        </span>
      ) : (
